...
public final class CrimsonIsleESP {
    private CrimsonIsleESP() {
    }

    public static void render(Minecraft client) {
        if (client.level == null || client.player == null || !SkyBlockESPConfig.INSTANCE.crimsonIsle.fairySouls) {
            return;
        }
        if (!CurrentIsland.isIsland("Crimson Isle")) {
            return;
        }

        try (var ignored = client.levelRenderer.collectPerFrameGizmos()) {
            for (BlockPos pos : FairySoulPositions.forIsland("Crimson Isle")) {
