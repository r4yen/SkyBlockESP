...
public final class MoongladeMarshESP {
    ...
    private MoongladeMarshESP() {
    }

    public static void render(Minecraft client) {
        SkyBlockESPConfig.FairySoulFloorDropIsland config = SkyBlockESPConfig.INSTANCE.moongladeMarsh;
        if (client.level == null || client.player == null || !hasAnyMoongladeMarshESPEnabled(config)) {
            return;
        }
        if (!CurrentIsland.isIsland("Moonglade Marsh") && !CurrentIsland.isIsland("Galatea")) {
            return;
        }

        try (var ignored = client.levelRenderer.collectPerFrameGizmos()) {
            if (config.fairySouls) {
                for (BlockPos pos : FairySoulPositions.forIsland("Moonglade Marsh")) {
    ...
    private static boolean hasAnyMoongladeMarshESPEnabled(SkyBlockESPConfig.FairySoulFloorDropIsland config) {
    ...
    private static List<AABB> findFloorDrops(Minecraft client) {
        ...
        for (Entity entity : client.level.getEntities(client.player, scanBox, MoongladeMarshESP::isFloorDropDisplay)) {
